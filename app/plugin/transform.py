...
def transform(req: HTTPRequest):
    ...
    This means that the urn:ngsi_ld:Workstation:1 signals that good parts were created,
    ...
    Multiply the counter by the PartsPerCycle value to get the actual value of the Job's counter. 
    Set the counter accordingly: the GoodPartCounter if "ct" contains "good", the RejectPartCounter otherwise.

   For example if the current Job is Job:202200045, the HTTPRequest will be transformed to the equivalent of
...
--data-raw '96'
    ...
    Steps:
        ...
        9. Get PartsPerCycle 
        10. Multiply "cv" by PartsPerCycle to get the Counter
        11. Check if the GoodPartCounter or the RejectPartCounter needs to be updated (transform["ct"])
